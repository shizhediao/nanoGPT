...
if __name__ == "__main__":
    folder_path = "/lustre/fsw/portfolios/nvr/users/sdiao/nanoGPT/out"
    specific_model_name = "out"
    raw_config_path = f"/lustre/fsw/portfolios/nvr/users/sdiao/adlr-pretrain/adlr-megatron-lm/regmix/mixture_config/config_nemotron_cc_ds244x_350m_r1-512"
    ...
    try:
        with open(csv_file, 'w', newline='') as csvfile:
            ...
            for model_name, model_results in results.items():
                ...
                # 加入pretraining valid loss
                cluster_id = model_name.split("_cluster_")[-1]
                print(f"cluster_id: {cluster_id}")
            
                # 从model_name中获取config名称
                config_name = model_name.split("_")[-1]
                row['config_name'] = config_name

                # 读取YAML配置文件
                config_path = f"{raw_config_path}/{config_name}.yaml"
                try:
                    print(f"SHIZHE DEBUG: READING config_path: {config_path}")
                    ...
                    print(f"Error reading config file for {config_name}: {e}")
